...
const ScreenHeader = ({
	...
	return (
		<div
			className={[
				`flex flex-col gap-5 rounded-2xl bg-[${Colors.cardBg}] p-4 shadow-[0_12px_32px_rgba(0,0,0,0.16)]`,
				...
				>
					...
					<div className="min-w-0 flex-1 md:ml-4">
						<div className="flex flex-wrap items-center gap-2">
							<h2
								className={[
									`m-0 font-bold leading-tight text-[${Colors.textPrimary}]`,
									titleClassName,
								].join(" ")}
							>
								{title}
						...
						{subtitle ? (
							<div className={`mt-2 text-sm text-[${Colors.textMuted}]`}>{subtitle}</div>
						) : null}
